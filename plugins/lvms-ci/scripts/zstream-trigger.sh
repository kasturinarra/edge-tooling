...
#
# Usage:
#   zstream-trigger.sh [--releases 4.14,4.16,...] [--dry-run] [--workdir /path]
#                      [--force] [--token-file /path/to/token]
#
# Environment:
#   GITHUB_TOKEN        Optional. GitHub PAT for higher API rate limits.
#   MY_APPCI_TOKEN      Required (unless --dry-run). Gangway API bearer token.
...
STATE_DIR="${HOME}/.cache/lvms-zstream"
STATE_FILE="${STATE_DIR}/last-tested.json"
...
Options:
  --releases LIST   Comma-separated releases (default: ${RELEASES})
  --dry-run         Print what would be triggered without calling Gangway
  --force           Trigger even if digest hasn't changed since last run
  --workdir DIR     Write summary JSON here (default: /tmp/lvms-zstream.<date>)
  --token-file F    Read MY_APPCI_TOKEN from file instead of environment
...
WORKDIR="${WORKDIR:-/tmp/lvms-zstream.$(date +%y%m%d)}"
mkdir -p "${WORKDIR}"
...
if ! ${DRY_RUN} && [[ -z "${MY_APPCI_TOKEN:-}" ]]; then
    err "MY_APPCI_TOKEN is required (set env or use --token-file)"
    exit 1
fi

# ---------------------------------------------------------------------------
# State management — track last-tested digest per release
# ---------------------------------------------------------------------------
...
get_last_digest() {
    local release="$1"
    jq -r --arg r "${release}" '.[$r].digest // ""' "${STATE_FILE}"
}

get_last_pr() {
    local release="$1"
    jq -r --arg r "${release}" '.[$r].pr // ""' "${STATE_FILE}"
}

save_digest() {
    local release="$1" digest="$2" pr_number="$3" image="$4"
    local tmp
    tmp=$(mktemp)
    jq --arg r "${release}" \
       --arg d "${digest}" \
       ...
       --arg i "${image}" \
       --arg t "$(date -Iseconds)" \
       '.[$r] = {digest: $d, pr: $p, image: $i, tested_at: $t}' \
...
resolve_snapshot_to_digest() {
    ...
    # Parse version prefix and timestamp from snapshot name
    # Format: lvm-operator-catalog-4-16-20260624-033025-000
    ...
    # Convert snapshot timestamp to epoch for comparison
    ...
    # Query quay for on-push build-image-index tags matching this version
    local tags_json
    ...
    # Find build-image-index tags created after the snapshot timestamp
    # Pick the closest one (smallest positive time delta)
    local result
...
process_release() {
    local release="$1"
    local version_dashed major minor
    major=$(echo "${release}" | cut -d. -f1)
    minor=$(echo "${release}" | cut -d. -f2)
    ...
    # Update state
    if ! ${DRY_RUN} && [[ ${triggered} -gt 0 ]]; then
        save_digest "${release}" "${digest}" "${pr_number}" "${image}"
    fi
...
main() {
    ...
    local total_triggered=0 total_skipped=0 total_errors=0
